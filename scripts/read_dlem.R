...
readDlemBiomeflux <- function(maskidx_file, dlem_biomeflux_file, pfts="all") {
  ...
  if(pfts=="all") {
    ...
    biomeflux_df_s <- lapply(pfts, genVarStack) %>% do.call(stack, .)
    names(biomeflux_df_s) <- outer(var_names, pfts, paste, sep="_") %>% as.vector
  }
...
## 
## read dlem gridflux output for inundated wetlands (Bowen's version)
readDlemGridfluxInun <- function(maskidx_file, dlem_gridflux_file) {
  file_size <- file.info(dlem_gridflux_file)$size
  raw <- readBin(dlem_gridflux_file, what="raw", n=file_size);
  ...
  var_names <- c("Findex_auto", "Findex_input", "year", "month", "day", 
                 ...
                 "g_lucc_cvrtN", "g_crop_manage_C", "g_crop_manage_N", "g_manureC", "g_manureN", 
                 "C_burntflux", "CH4_burntflux", "CO_burntflux", "CO2_burntflux", "NMHC_burntflux", 
                 "g_litleachDOC", "g_leach_DOC",
                 "g_leach_POC", "g_leach_DIC", "g_ch4Wet", "g_ch4Sum", "g_ch4", "g_VOCisop", "g_VOCmono", "g_VOCovoc",
                 "g_VOCorvoc", "g_VOCco","g_nuptake", "g_NMin", "g_leakN", "g_leachNH4", 
                 "g_leachNO3", "g_litleachDON", "g_leachDON", "g_leachPON", 
                 "g_denitrif", "g_nitrif", "g_nFix", "g_Ndep", "g_NFer", "g_n2o", "g_no",
                 "g_nh3Emisn", "N_burntflux", "N2O_burntflux", "NOy_burntflux", "NH3_burntflux",
                 "g_irrigation", "g_irrigation_withd", "g_evap",
                 "g_infil", "g_trans","g_runoff_drain","g_runoff_surf",
                 "g_stream_in", "g_stream_out","g_stream_et","g_stream_irrig",
                 "g_stream_outDOC", "g_stream_outPOC","g_stream_outNH4",
                 "g_stream_outNO3","g_stream_outDIN", "g_stream_outDON", "g_stream_outPON",
...
## read dlem gridpool output for inundated wetlands (Bowen's version)
readDlemGridpoolInun <- function(maskidx_file, dlem_gridpool_file) {
  
  ## read dlem gridpool using raw method
  file_size <- file.info(dlem_gridpool_file)$size
  raw <- readBin(dlem_gridpool_file, what="raw", n=file_size);
  
  nbr_of_records <- length(raw) %/% 17;
  raw <- matrix(raw, nrow=17*4, byrow=FALSE)
  ...
  data_float2 <- lapply(7:17, FUN=function(var) {
    floats <- readBin(con=as.vector(raw[(4*(var - 1) + 1): (4*var), ]), what="double", size=4,
                      n=nbr_of_records/4)
    return(floats)
  })
  ...
  var_names <- c("Findex_auto", "Findex_input","Farea", "year", "month", "day", 
                 "VegC_rec", "LitC_rec", "SoilC_rec", "WoodagC_rec",
                 "VegN_rec", "LitN_rec", "SoilN_rec", "FL_fine_rec", "FL_coarse_rec",
                 "FL_livewood_rec", "FL_liveherb_rec")
...
## read dlem biomestate output
readDlemBiomestateInun <- function(maskidx_file, dlem_biomestate_file) {
  ...
  ## var list
  var_names <- c("Findex_auto", "Findex_input", "year", "month", "day", 
                 "ecoid","pft","fbiomec", "ecoage", "cleafmax_pot", "laiphenmax",
                 "g_suno3_pday", "g_shao3_pday", "canopywater", "leaf_c", 
                 "sapw_c","heartw_c","froot_c","croot_c",
                 "reprod_c", "wdag_c", "wdbg_c", "aom1ag_c", "aom1bg_c", 
                 "aom2ag_c", "aom2bg_c", "smb1_c",
                 "smb2_c", "smr_c","nom_c", "psom_c", "soil_doc", "soil_dic",
                 "soil_ch4", "inunSoil_ch4", "leaf_n", "sapw_n", "heartw_n", "froot_n",
                 "croot_n", "reprod_n", "wdag_n", "wdbg_n", "aom1ag_n", 
                 "aom1bg_n", "aom2ag_n", "aom2bg_n", "smb1_n", "smb2_n", "smr_n", "nom_n",
                 "psom_n", "soil_don", "av_NH4", "av_NO3"
  )
  names(biomestate_df) <- var_names
  ...
  biomestate_df_s <- lapply(91, genVarStack) %>% do.call(stack, .) ##unique(biomestate_df$ecoid)
  names(biomestate_df_s) <- outer(var_names, 91, paste, sep="_") %>% as.vector ##unique(biomestate_df$ecoid)
  
  return(biomestate_df_s)
}

## seperate nitrificaiton and denitrification
## read dlem gridflux output
readDlemGridfluxNit <- function(maskidx_file, dlem_gridflux_file) {
  file_size <- file.info(dlem_gridflux_file)$size
  raw <- readBin(dlem_gridflux_file, what="raw", n=file_size);
  
  nbr_of_records <- length(raw) %/% 81;
  raw <-  matrix(raw, nrow=81*4, byrow=FALSE)
  ...
  data_float <- lapply(6:81, FUN=function(var) {
    floats <- readBin(con=as.vector(raw[(4*(var - 1) + 1): (4*var), ]), what="double", size=4,
                      n=nbr_of_records/4)
    return(floats)
  })
  ...
  var_names <- c("Findex_auto", "Findex_input", "year", "month", "day", 
                 ...
                 "g_VOCorvoc", "g_VOCco","g_nuptake", "g_NMin", "g_leakN", "g_leachNH4", 
                 "g_leachNO3", "g_litleachDON", "g_leachDON", "g_leachPON", 
                 "g_denitrif", "g_nitrif", "g_nFix", "g_Ndep", "g_NFer", "g_n2o",
                 "g_n2o_nitrif", "g_n2o_denitrif", "g_no",
                 "g_nh3Emisn", "N_burntflux", "N2O_burntflux", "NOy_burntflux", "NH3_burntflux",
                 "N_burntmort","g_irrigation", "g_irrigation_withd", "g_evap",
                 "g_infil", "g_trans","g_runoff_drain","g_runoff_surf",
                 "g_stream_in", "g_stream_out","g_stream_et","g_stream_irrig",
...
##read tropical forest flux
readDlemTroForestflux <- function(maskidx_file, dlem_biomeflux_file, biome_ids) {
  ...
  biomeflux_df_s <- lapply(biome_ids, genVarStack) %>% do.call(stack, .)
  names(biomeflux_df_s) <- outer(var_names, biome_ids, paste, sep="_") %>% as.vector
  
  return(biomeflux_df_s)
}


##read tropical forest biome state
## read dlem biomestate output
readDlemTroForeststate <- function(maskidx_file, dlem_biomestate_file, biome_ids) {
  ...
  biomestate_df_s <- lapply(biome_ids, genVarStack) %>% do.call(stack, .) ##unique(biomestate_df$ecoid)
  names(biomestate_df_s) <- outer(var_names, biome_ids, paste, sep="_") %>% as.vector ##unique(biomestate_df$ecoid)
