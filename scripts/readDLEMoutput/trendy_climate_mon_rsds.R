## Title: trendy_climate month dswrf,rsds
## Date:2018-08-23
...
## set workspace
ori_dir <- setwd("//spirit/sfws_edge/hao_shi/gcp18/trendyv7/clim/dswrf/")
...
time <- 1:(118 * 12) - 1
time_dim <- ncdim_def("time", "months since 1900-01-015 00:00:00", time, calendar="noleap")
...
var_name <- c("rsds")
var_unit <- "W m-2"
var_lname <- "Surface Downwelling Shortwave Radiation"

var_def <- ncvar_def(var_name, var_unit, list(lon_dim, lat_dim, time_dim), fill_value,
                     var_lname, "float", compression=9)

## create netcdf
nc_file <- "G:\\trendy_2018\\rsds\\DLEM_S3_rsds.nc" ## note: set up the path for nc_file
